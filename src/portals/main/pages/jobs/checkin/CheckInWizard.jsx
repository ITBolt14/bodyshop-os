// ===============================================
// BODYSHOP OS - Check-In Wizard: Main Controller
// ===============================================
...
export function CheckInWizard() {
  ...
  const handleSave = async (finalStatus) => {
    ...
    try {
      ...
      console.log('Step 1 - Vehicle ID:', vehicleId)
      ...
      if (vehicleId) {
        ...
        console.log('Vehicle update error:', vErr)
        ...
        console.log('Vehicle insert result:', newVehicle, 'error:', vErr)
        if (vErr) throw new Error(`Vehicle insert failed: ${vErr.message}`)
        vehicleId = newVehicle.id
      }

      console.log('Step 1 complete - vehicleId:', vehicleId)
      ...
      console.log('Job number result:', jobNumData, 'error:', jnErr)
      if (jnErr) throw new Error(`Job number failed: ${jnErr.message}`)

      // STEP 3 - Insert job
      console.log('Step 3 - Inserting job...')
      ...
      console.log('Job insert result:', newJob, 'error:', jobErr)
      if (jobErr) throw new Error(`Job insert failed: ${jobErr.message}`)
        ...
        if (jobData.job_type === 'insurance') {
          console.log('Step 4 - Inserting claim...')
          ...
          console.log('Claim insert error:', claimErr)
          if (claimErr) throw new Error(`Claim insert failed: ${claimErr.message}`)
        }

        // STEP 5 - Write audit log
        console.log('Step 5 - Writing audit log...')
