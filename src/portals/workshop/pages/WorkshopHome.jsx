...
// Job search with on-screen keyboard
// ===============================================
...
import {
  ...
  Car, User, CornerDownLeft
} from 'lucide-react'

// SECTION: On-Screen Keyboard
const KB_ROWS = [
  ['Q','W','E','R','T','Y','U','I','O','P'],
  ['A','S','D','F','G','H','J','K','L'],
  ['Z','X','C','V','B','N','M'],
...
function OnScreenKeyboard({ onKey, onDelete, onSearch, disabled }) {
  ...
  const KeyBtn = ({ label, wide = false, variant = 'default' }) => (
    <button
      onClick={() => {
        if (variant === 'delete') { onDelete(); return }
        if (variant === 'search') { onSearch(); return }
        onKey(label)
      }}
      disabled={disabled}
      className={`flex items-center justify-center rounded-xl
                  font-bold transition-all duration-100
                  active:scale-95 select-none h-12 text-sm
                  disabled:opacity-40
                  ${wide ? 'px-4' : 'flex-1'}
                  ${variant === 'delete'
                    ? 'bg-gray-700 hover:bg-gray-600 text-gray-300 px-4'
                    : variant === 'search'
                    ? 'bg-brand-600 hover:bg-brand-500 text-white px-4'
  ...
  return (
    <div className="space-y-2 mt-3">
      ...
      {/* Bottom row - space, delete, search */}
...
export function WorkshopHome() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-5 py-3
        ...
        <button
          ...
          className="flex items-center gap-2 text-gray-500
                     hover:text-white transition-colors text-sm py-2 px-3
      ...
      <div className="flex-1 flex flex-col px-4 py-4 max-w-2xl
                      mx-auto w-full">

        {/* Search bar - display only, input via keyboard */}
          ...
          <div className="mt-4 space-y-2">
            {results.map(job => (
              <button
                ...
                className="w-full bg-gray-800 hover:bg-gray-750
                           border border-gray-700 hover:border-brand-500
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs font-bold bg-gray-700
